...
MIGRATION_REGISTRY = [
    ...
    {
        "key": "019_recital_production_center",
        "title": "Performance Lineup and Recital Production Center",
        "description": "Live show sequencing, dancers, backstage assignments, cues, checklists, and stage management.",
        "required_tables": ["production_show_state","production_performance_students","production_performance_cues","production_assignments","production_checklist_items"],
        "required_columns": {
            "production_show_state": ["show_id","current_performance_id","live_status","intermission_active","delay_minutes","emergency_note"],
            "production_performance_students": ["performance_id","student_id","quick_change","dressing_room","backstage_note"],
            "production_performance_cues": ["performance_id","lights_cue","music_cue","curtain_cue","props_cue","stage_note"],
            "production_assignments": ["show_id","performance_id","assignment_type","person_name","location","status"],
            "production_checklist_items": ["show_id","category","item_text","completed","sort_order"]
        },
    },
...
def sync_production_students(connection, show_id):
    performances=connection.execute("SELECT id,class_id FROM recital_performances WHERE show_id=?",(show_id,)).fetchall(); added=0
    for p in performances:
        if not p["class_id"]: continue
        students=connection.execute("SELECT student_id FROM class_enrollments WHERE class_id=? AND status='Active'",(p["class_id"],)).fetchall()
        for s in students:
            exists=connection.execute("SELECT id FROM production_performance_students WHERE performance_id=? AND student_id=?",(p["id"],s["student_id"])).fetchone()
            if not exists:
                connection.execute("INSERT INTO production_performance_students(performance_id,student_id,assignment_source) VALUES (?,?,'Class')",(p["id"],s["student_id"])); added+=1
    return added

@app.route("/admin/production")
@permission_required("production")
def production_center():
    c=get_db(); rows=c.execute("""SELECT s.id,s.name,s.show_date,s.start_time,p.name AS production_name,
      COUNT(DISTINCT rp.id) AS performance_count,COALESCE(SUM(CASE WHEN rp.music_status='Ready' THEN 1 ELSE 0 END),0) AS music_ready,
      COALESCE(pss.live_status,'Not Started') AS live_status
      FROM recital_shows s JOIN recital_productions p ON p.id=s.production_id LEFT JOIN recital_performances rp ON rp.show_id=s.id
      LEFT JOIN production_show_state pss ON pss.show_id=s.id GROUP BY s.id,s.name,s.show_date,s.start_time,p.name,pss.live_status
      ORDER BY s.show_date DESC,s.start_time""").fetchall(); c.close()
    return render_template('production_center.html',shows=[dict(x) for x in rows])

@app.route("/admin/production/shows/<int:show_id>")
@permission_required("production")
def production_show(show_id):
    c=get_db(); show=c.execute("SELECT s.*,p.name AS production_name,p.venue FROM recital_shows s JOIN recital_productions p ON p.id=s.production_id WHERE s.id=?",(show_id,)).fetchone()
    if not show: c.close(); return ('Show not found',404)
    c.execute("INSERT INTO production_show_state(show_id) VALUES (?) ON CONFLICT(show_id) DO NOTHING",(show_id,)); sync_production_students(c,show_id); c.commit()
    perf=c.execute("""SELECT rp.*,cl.name AS class_name,COUNT(DISTINCT pps.student_id) AS dancer_count,
      COALESCE(SUM(CASE WHEN pps.quick_change=1 THEN 1 ELSE 0 END),0) AS quick_change_count,
      pc.lights_cue,pc.music_cue,pc.curtain_cue,pc.props_cue
      FROM recital_performances rp LEFT JOIN classes cl ON cl.id=rp.class_id LEFT JOIN production_performance_students pps ON pps.performance_id=rp.id
      LEFT JOIN production_performance_cues pc ON pc.performance_id=rp.id WHERE rp.show_id=?
      GROUP BY rp.id,rp.show_id,rp.class_id,rp.title,rp.performance_order,rp.performance_type,rp.duration_seconds,rp.music_title,rp.music_url,rp.music_status,rp.entrance_notes,rp.exit_notes,rp.costume_notes,rp.status,rp.created_at,rp.updated_at,cl.name,pc.lights_cue,pc.music_cue,pc.curtain_cue,pc.props_cue ORDER BY rp.performance_order,rp.id""",(show_id,)).fetchall()
    ass=c.execute("SELECT pa.*,rp.title AS performance_title FROM production_assignments pa LEFT JOIN recital_performances rp ON rp.id=pa.performance_id WHERE pa.show_id=? ORDER BY pa.assignment_type,pa.person_name",(show_id,)).fetchall()
    chk=c.execute("SELECT * FROM production_checklist_items WHERE show_id=? ORDER BY category,sort_order,id",(show_id,)).fetchall(); c.close()
    return render_template('production_show.html',show=dict(show),performances=[dict(x) for x in perf],assignments=[dict(x) for x in ass],checklist=[dict(x) for x in chk])

@app.route("/admin/production/shows/<int:show_id>/sync",methods=["POST"])
@permission_required("production")
def production_sync(show_id):
    c=get_db(); added=sync_production_students(c,show_id); c.commit(); c.close(); flash(f'{added} new dancer assignment(s) synchronized.','success'); return redirect(url_for('production_show',show_id=show_id))

@app.route("/admin/production/performances/<int:performance_id>")
@permission_required("production")
def production_performance(performance_id):
    c=get_db(); p=c.execute("""SELECT rp.*,rs.name AS show_name,p.name AS production_name,cl.name AS class_name FROM recital_performances rp
      JOIN recital_shows rs ON rs.id=rp.show_id JOIN recital_productions p ON p.id=rs.production_id LEFT JOIN classes cl ON cl.id=rp.class_id WHERE rp.id=?""",(performance_id,)).fetchone()
    if not p: c.close(); return ('Performance not found',404)
    dancers=c.execute("""SELECT pps.*,s.first_name,s.last_name,s.preferred_name FROM production_performance_students pps JOIN students s ON s.id=pps.student_id WHERE pps.performance_id=? ORDER BY s.last_name,s.first_name""",(performance_id,)).fetchall()
    cues=c.execute("SELECT * FROM production_performance_cues WHERE performance_id=?",(performance_id,)).fetchone(); students=c.execute("SELECT id,first_name,last_name,preferred_name FROM students WHERE status='Active' ORDER BY last_name,first_name").fetchall(); c.close()
    return render_template('production_performance.html',performance=dict(p),dancers=[dict(x) for x in dancers],cues=dict(cues) if cues else {},all_students=[dict(x) for x in students])

@app.route("/admin/production/performances/<int:performance_id>/cues",methods=["POST"])
@permission_required("production")
def production_save_cues(performance_id):
    c=get_db(); f=request.form; c.execute("""INSERT INTO production_performance_cues(performance_id,lights_cue,music_cue,curtain_cue,props_cue,stage_note,photo_allowed,video_allowed) VALUES (?,?,?,?,?,?,?,?) ON CONFLICT(performance_id) DO UPDATE SET lights_cue=excluded.lights_cue,music_cue=excluded.music_cue,curtain_cue=excluded.curtain_cue,props_cue=excluded.props_cue,stage_note=excluded.stage_note,photo_allowed=excluded.photo_allowed,video_allowed=excluded.video_allowed,updated_at=CURRENT_TIMESTAMP""",(performance_id,f.get('lights_cue',''),f.get('music_cue',''),f.get('curtain_cue',''),f.get('props_cue',''),f.get('stage_note',''),1 if f.get('photo_allowed')=='on' else 0,1 if f.get('video_allowed')=='on' else 0)); c.commit(); c.close(); flash('Stage cues saved.','success'); return redirect(url_for('production_performance',performance_id=performance_id))

@app.route("/admin/production/performances/<int:performance_id>/dancers/add",methods=["POST"])
@permission_required("production")
def production_add_dancer(performance_id):
    c=get_db(); sid=int(request.form.get('student_id','0') or 0); c.execute("INSERT INTO production_performance_students(performance_id,student_id,assignment_source) VALUES (?,?,'Manual') ON CONFLICT(performance_id,student_id) DO NOTHING",(performance_id,sid)); c.commit(); c.close(); return redirect(url_for('production_performance',performance_id=performance_id))

@app.route("/admin/production/dancers/<int:assignment_id>/update",methods=["POST"])
@permission_required("production")
def production_update_dancer(assignment_id):
    c=get_db(); row=c.execute("SELECT performance_id FROM production_performance_students WHERE id=?",(assignment_id,)).fetchone(); f=request.form
    c.execute("UPDATE production_performance_students SET quick_change=?,dressing_room=?,backstage_note=? WHERE id=?",(1 if f.get('quick_change')=='on' else 0,f.get('dressing_room',''),f.get('backstage_note',''),assignment_id)); c.commit(); pid=int(row['performance_id']); c.close(); return redirect(url_for('production_performance',performance_id=pid))

@app.route("/admin/production/shows/<int:show_id>/assignments",methods=["POST"])
@permission_required("production")
def production_add_assignment(show_id):
    f=request.form; pv=f.get('performance_id','').strip(); c=get_db(); c.execute("INSERT INTO production_assignments(show_id,performance_id,assignment_type,person_name,location,phone,notes,status) VALUES (?,?,?,?,?,?,?,?)",(show_id,int(pv) if pv else None,f.get('assignment_type','Volunteer'),f.get('person_name',''),f.get('location',''),f.get('phone',''),f.get('notes',''),f.get('status','Assigned'))); c.commit(); c.close(); return redirect(url_for('production_show',show_id=show_id))

@app.route("/admin/production/shows/<int:show_id>/checklist",methods=["POST"])
@permission_required("production")
def production_add_checklist(show_id):
    f=request.form;c=get_db();c.execute("INSERT INTO production_checklist_items(show_id,category,item_text,notes,sort_order) VALUES (?,?,?,?,?)",(show_id,f.get('category','General'),f.get('item_text',''),f.get('notes',''),int(f.get('sort_order','0') or 0)));c.commit();c.close();return redirect(url_for('production_show',show_id=show_id))

@app.route("/admin/production/checklist/<int:item_id>/toggle",methods=["POST"])
@permission_required("production")
def production_toggle_checklist(item_id):
    c=get_db(); row=c.execute("SELECT show_id,completed FROM production_checklist_items WHERE id=?",(item_id,)).fetchone(); new=0 if int(row['completed'] or 0) else 1; c.execute("UPDATE production_checklist_items SET completed=?,completed_at=CASE WHEN ?=1 THEN CURRENT_TIMESTAMP ELSE NULL END,completed_by=CASE WHEN ?=1 THEN ? ELSE NULL END WHERE id=?",(new,new,new,int(session.get('admin_user_id') or 0) or None,item_id));c.commit();sid=int(row['show_id']);c.close();return redirect(url_for('production_show',show_id=sid))

@app.route("/admin/production/shows/<int:show_id>/live")
@permission_required("production")
def production_live(show_id):
    c=get_db(); show=c.execute("SELECT s.*,p.name AS production_name FROM recital_shows s JOIN recital_productions p ON p.id=s.production_id WHERE s.id=?",(show_id,)).fetchone(); c.execute("INSERT INTO production_show_state(show_id) VALUES (?) ON CONFLICT(show_id) DO NOTHING",(show_id,));c.commit(); state=c.execute("SELECT * FROM production_show_state WHERE show_id=?",(show_id,)).fetchone(); rows=c.execute("""SELECT rp.*,cl.name AS class_name,pc.lights_cue,pc.music_cue,pc.curtain_cue,pc.props_cue,pc.stage_note FROM recital_performances rp LEFT JOIN classes cl ON cl.id=rp.class_id LEFT JOIN production_performance_cues pc ON pc.performance_id=rp.id WHERE rp.show_id=? ORDER BY rp.performance_order,rp.id""",(show_id,)).fetchall(); perfs=[dict(x) for x in rows]; cur=-1
    for i,p in enumerate(perfs):
        if state['current_performance_id'] and int(p['id'])==int(state['current_performance_id']): cur=i;break
    current=perfs[cur] if cur>=0 else None; nxt=perfs[cur+1] if cur+1<len(perfs) else (perfs[0] if cur<0 and perfs else None); hole=perfs[cur+2] if cur+2<len(perfs) else None;c.close();return render_template('production_live.html',show=dict(show),state=dict(state),current=current,next_performance=nxt,in_hole=hole,performances=perfs)

@app.route("/admin/production/shows/<int:show_id>/live/advance",methods=["POST"])
@permission_required("production")
def production_advance(show_id):
    c=get_db(); ids=[int(x['id']) for x in c.execute("SELECT id FROM recital_performances WHERE show_id=? ORDER BY performance_order,id",(show_id,)).fetchall()]; state=c.execute("SELECT * FROM production_show_state WHERE show_id=?",(show_id,)).fetchone(); current=state['current_performance_id'] if state else None
    if not ids: c.close(); return redirect(url_for('production_live',show_id=show_id))
    if current is None: nxt=ids[0]; status='Running'
    else:
        try: idx=ids.index(int(current))
        except ValueError: idx=-1
        nxt=ids[idx+1] if idx+1<len(ids) else None; status='Running' if nxt else 'Completed'
    c.execute("""INSERT INTO production_show_state(show_id,current_performance_id,live_status,started_at) VALUES (?,?,?,CURRENT_TIMESTAMP) ON CONFLICT(show_id) DO UPDATE SET current_performance_id=excluded.current_performance_id,live_status=excluded.live_status,started_at=COALESCE(production_show_state.started_at,CURRENT_TIMESTAMP),updated_at=CURRENT_TIMESTAMP""",(show_id,nxt,status));c.commit();c.close();return redirect(url_for('production_live',show_id=show_id))

@app.route("/admin/production/shows/<int:show_id>/live/state",methods=["POST"])
@permission_required("production")
def production_state(show_id):
    f=request.form;c=get_db();c.execute("""INSERT INTO production_show_state(show_id,live_status,delay_minutes,emergency_note,intermission_active) VALUES (?,?,?,?,?) ON CONFLICT(show_id) DO UPDATE SET live_status=excluded.live_status,delay_minutes=excluded.delay_minutes,emergency_note=excluded.emergency_note,intermission_active=excluded.intermission_active,updated_at=CURRENT_TIMESTAMP""",(show_id,f.get('live_status','Running'),int(f.get('delay_minutes','0') or 0),f.get('emergency_note',''),1 if f.get('intermission_active')=='on' else 0));c.commit();c.close();return redirect(url_for('production_live',show_id=show_id))
