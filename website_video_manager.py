...
MAX_VIDEO_BYTES = 450 * 1024 * 1024
MAX_REQUEST_BYTES = 460 * 1024 * 1024
...
def _save_video(file_storage) -> str:
    ...
    if size > MAX_VIDEO_BYTES:
        target.unlink()
        raise ValueError("That media file is larger than 450 MB. Please use a smaller file.")
...
def register_website_video_manager(app, permission_required, log_activity=None) -> None:
    ...
    def website_media_too_large(_error):
        flash(
            "That upload is too large. Website videos and GIFs can be up to 450 MB; photos can be up to 25 MB.",
            "error",
        )
